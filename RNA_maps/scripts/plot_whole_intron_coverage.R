...
x_breaks <- c( 1+(exon_length/2), 
               ...
               1+exon_length+flank+intron_length+flank+(exon_length/2),
...
p <- ggplot() + theme_classic() +
  ...
  geom_vline( data = data.frame(xintercept = x_breaks[c(2:5,9:12)], slope=0), aes(xintercept=xintercept),colour = "black", linetype="dashed",alpha=0.5 )
